...
cummins_data = {
    ...
    'Revenue': [4200, 4100, 4500, 5030, 5670, 6410],  # Millions USD
    'EBITDA': [220, 296, 390, 640, 1007, 1265],  # Estimated from quarterly data
    'EBITDA_Margin': [5.2, 7.2, 8.7, 12.7, 17.8, 19.7]  # Percent
}
...
caterpillar_data = {
    'Year': [2019, 2020, 2021, 2022, 2023, 2024],
    'Revenue': [20870, 19231, 22831, 26330, 28000, 28850],  # Millions USD
    'Operating_Profit': [2088, 2100, 2623, 4184, 5310, 5750],  # Segment profit
    'EBITDA_Margin': [10.0, 10.9, 11.5, 15.9, 19.0, 19.9]  # Percent
}
...
ax1.set_xticks(x)
ax1.set_xticklabels(df_cummins['Year'])
...
ax2.set_xticks(x)
ax2.set_xticklabels(df_cummins['Year'])
...
ax3 = axes[1, 0]
ax3.plot(df_cummins['Year'], df_cummins['EBITDA_Margin'], marker='o', linewidth=3,
         markersize=10, label='Cummins Power Systems', color=cummins_secondary)
ax3.plot(df_cat['Year'], df_cat['EBITDA_Margin'], marker='s', linewidth=3,
...
for i, (year, margin) in enumerate(zip(df_cummins['Year'], df_cummins['EBITDA_Margin'])):
    ax3.text(year, margin + 0.5, f'{margin:.1f}%', ha='center', va='bottom',
             fontsize=9, fontweight='bold', color=cummins_secondary)

for i, (year, margin) in enumerate(zip(df_cat['Year'], df_cat['EBITDA_Margin'])):
    ax3.text(year, margin - 1.2, f'{margin:.1f}%', ha='center', va='top',
             fontsize=9, fontweight='bold', color=cat_secondary)
...
ax4.set_xticks(x)
ax4.set_xticklabels(df_cummins['Year'])
...
print("\nKEY INSIGHTS:")
print("-" * 80)
print(f"1. Revenue Scale:")
print(f"   - Caterpillar E&T is ~{df_cat['Revenue'].iloc[-1]/df_cummins['Revenue'].iloc[-1]:.1f}x larger than Cummins Power Systems")
print(f"   - CAT 2024: ${df_cat['Revenue'].iloc[-1]/1000:.1f}B | Cummins 2024: ${df_cummins['Revenue'].iloc[-1]/1000:.1f}B")

print(f"\n2. Profitability (2024):")
...
cummins_cagr = ((df_cummins['Revenue'].iloc[-1] / df_cummins['Revenue'].iloc[0]) ** (1/5) - 1) * 100
cat_cagr = ((df_cat['Revenue'].iloc[-1] / df_cat['Revenue'].iloc[0]) ** (1/5) - 1) * 100
print(f"\n3. Revenue CAGR (2019-2024):")
print(f"   - Cummins: {cummins_cagr:.1f}%")
print(f"   - Caterpillar: {cat_cagr:.1f}%")

print(f"\n4. Margin Improvement (2019-2024):")
